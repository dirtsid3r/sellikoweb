...
const getUserRole = (user: any): string => {
  return (user?.user_role || user?.role || '').toString().toUpperCase()
}
...
function VendorManagement({ availableCities, configLoading }: {
...
function AgentManagement({ availableCities, configLoading }: {
  availableCities: string[]
  configLoading: boolean
}) {
  ...
  const isCurrentUserAdmin = getUserRole(user) === 'ADMIN'
    ...
    const fetchFormConfig = async () => {
      try {
        ...
        if (config.success) {
          ...
          console.error('❌ [ADMIN-SETTINGS] Failed to load form config:', config.error)
          // Fallback to empty array 
          setAvailableCities([])
        }
      } catch (error) {
        console.error('💥 [ADMIN-SETTINGS] Error fetching form config:', error)
        // Fallback to empty array
        setAvailableCities([])
      } finally {
